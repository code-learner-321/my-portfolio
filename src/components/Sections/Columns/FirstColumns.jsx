...
import columnFirst from "../../../datas/columnOne.json";
...
function FirstColumns() {
  const firstColBoxesAll = useRef([]);

  useEffect(() => {
    firstColBoxesAll.current.forEach((firstBox, index) => {
      gsap.fromTo(
        firstBox,
        { x: -100, opacity: 0 },
        {
  ...
  return (
    <div className="flex flex-col gap-2 p-1">
      ...
      {columnFirst.map((data,index) => {
        return (
          <div key={data.id}>
            <div ref={ e => firstColBoxesAll.current[index] = e } className="firstBox max-w-sm rounded overflow-hidden shadow-2xl">
